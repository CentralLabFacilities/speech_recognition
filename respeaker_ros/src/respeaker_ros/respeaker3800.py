...
EXTRA_PARAMETER3800 = {
    "AEC_FIXEDBEAMSAZIMUTH_VALUES_BEAM1": ("float", "Azimuth values in radians for beams in fixed mode - fixed beam 1", 3.14, 0.00, 6.28318),
    "AEC_FIXEDBEAMSELEVATION_VALUES_BEAM1": ("float", "Elevation angles in radians for the beams in fixed mode. - fixed beam 1", 0, 0.00, 6.28318),
    "AEC_FIXEDBEAMSAZIMUTH_VALUES_BEAM2": ("float", "Azimuth values in radians for beams in fixed mode - fixed beam 2", 3.14, 0.00, 6.28318),
    "AEC_FIXEDBEAMSELEVATION_VALUES_BEAM2": ("float", "Elevation angles in radians for the beams in fixed mode. - fixed beam 2",0 , 0.00, 6.28318),
}
...
PARAMETERS_3800 = {
    ...
    # AEC_RESID commands
    "SHF_BYPASS": (33, 70, 1, "rw", "uint8", "AEC bypass", 0, 1),
    ...
    "AEC_FIXEDBEAMSGATING": (33, 83, 1, "rw", "uint8", "Enables/disables gating for beams in fixed mode. When enabled, speech energy is used to determine which beam should be active; inactive beams are silenced.",0,1),
    ...
    "AEC_AECPATHCHANGE": (33, 0, 1, "ro", "int32", "AEC Path Change Detection. Valid range: 0,1 (false,true)", 0, 1),
    "AEC_HPFONOFF": (33, 1, 1, "rw", "int32", "High-pass Filter on microphone signals. Valid range: 0,1,2,3,4 (0:Off, 1:on70, 2:on125, 3:on150, 4:on180)", 0, 4),
    ...
    "AEC_ASROUTONOFF": (33, 35, 1, "rw", "int32", "Enables the automatic speech recognition (ASR) output: if set to 0, the AEC residuals are output, one channel per microphone, if set to 1, the ASR processed output is used, where each channel is associated with a beam from the beamformer. Valid range: 0,1 (off, on)", 0, 1),
    "AEC_ASROUTGAIN": (33, 36, 1, "rw", "float", "Fixed gain applied to asr output, i.e., only applied if ASROUTONOFF==1.. Valid range: [0.0 .. 1000.0] (linear gain factor)", 0.0, 1000.0),
    "AEC_FIXEDBEAMSONOFF": (33, 37, 1, "rw", "int32", "Enables or disables fixed focused beam mode. Valid range: 0,1 (off, on)", 0,1),
    ...
    "AUDIO_MGR_MIC_GAIN": (35, 0, 1, "rw", "float", "Audio Mgr pre SHF microphone gain", -999.0, 999.0),
    "AUDIO_MGR_REF_GAIN": (35, 1, 1, "rw", "float", "Audio Mgr pre SHF reference gain", -999.0, 999.0),
    ...
    "AUDIO_MGR_FAR_END_DSP_ENABLE": (35, 25, 1, "rw", "uint8", "Enables/disables XVF3800 far-end DSP (if implemented). Write a 1 to enable, 0 to disable", 0, 1),
    ...
    "LED_BRIGHTNESS": (20, 13, 1, "rw", "uint8", "Set the brightness value of the LED for the breath and rainbow mode", 0, 255),
    "LED_GAMMIFY": (20, 14, 1, "rw", "uint8", "Set to enable gamma correction, 0 = disable, 1 = enable", 0, 1),
    ...
    "PP_AGCONOFF": (17, 10, 1, "rw", "int32", "Automatic Gain Control. Valid range: 0,1 (off,on)", 0,1),
    "PP_AGCMAXGAIN": (17, 11, 1, "rw", "float", "Maximum AGC gain factor. Valid range: [1.0 .. 1000.0] (linear gain factor)", 1.0, 1000.0),
    "PP_AGCDESIREDLEVEL": (17, 12, 1, "rw", "float", "Target power level of the output signal. Valid range: [1e-8 .. 1.0] (power level)", 1e-8, 1.0),
    "PP_AGCGAIN": (17, 13, 1, "rw", "float", "Current AGC gain factor. Valid range: [1.0 .. 1000.0] (linear gain factor)", 1.0, 1000.0),
    "PP_AGCTIME": (17, 14, 1, "rw", "float", "Ramp-up/down time-constant. Valid range: [0.5 .. 4.0] (seconds)", 0.5, 4.0),
    "PP_AGCFASTTIME": (17, 15, 1, "rw", "float", "Ramp down time-constant in case of peaks. Valid range: [0.05 .. 4.0] (seconds)", 0.05, 4.0),
    "PP_AGCALPHAFASTGAIN": (17, 16, 1, "rw", "float", "Gain threshold enabling fast alpha mode. Valid range: [0.0 .. 1000.0] (linear gain factor)", 0.0, 1000.0),
    "PP_AGCALPHASLOW": (17, 17, 1, "rw", "float", "Slow memory parameter for speech power. Valid range [0.0 .. 1.0]", 0.0,1.0),
    "PP_AGCALPHAFAST": (17, 18, 1, "rw", "float", "Fast memory parameter for speech power. Valid range [0.0 .. 1.0]", 0.0,1.0),
    "PP_LIMITONOFF": (17, 19, 1, "rw", "int32", "Limiter on communication output. Valid range: 0,1 (off,on)", 0,1),
    "PP_LIMITPLIMIT": (17, 20, 1, "rw", "float", "Maximum limiter power. Valid range: [1e-8 .. 1.0] (power level)", 1e-8,1.0),
    "PP_MIN_NS": (17, 21, 1, "rw", "float", "Gain-floor for stationary noise suppression. Valid range: [0.0 .. 1.0]", 0.0,1.0),
    "PP_MIN_NN": (17, 22, 1, "rw", "float", "Gain-floor for non-stationary noise suppression. Valid range: [0.0 .. 1.0]", 0.0,1.0),
    "PP_ECHOONOFF": (17, 23, 1, "rw", "int32", "Echo suppression. Valid range: 0,1 (off,on)", 0,1),
    "PP_GAMMA_E": (17, 24, 1, "rw", "float", "Over-subtraction factor of echo (direct and early components). Valid range: [0.0 .. 2.0]", 0.0,2.0),
    "PP_GAMMA_ETAIL": (17, 25, 1, "rw", "float", "Over-subtraction factor of echo (tail components). Valid range: [0.0 .. 2.0]", 0.0,2.0),
    "PP_GAMMA_ENL": (17, 26, 1, "rw", "float", "Over-subtraction factor of non-linear echo. Valid range: [0.0 .. 5.0]", 0.0,5.0),
    "PP_NLATTENONOFF": (17, 27, 1, "rw", "int32", "Non-Linear echo attenuation. Valid range: 0,1 (off,on)", 0,1),
    "PP_NLAEC_MODE": (17, 28, 1, "rw", "int32", "Non-Linear AEC training mode. Valid range: 0,1,2 (normal,train,train2)",0,2),
    ...
    "PP_FMIN_SPEINDEX": (17, 30, 1, "rw", "float", "In case of double talk, frequencies below SPEINDEX are more suppressed than frequencies above SPEINDEX. The actual suppression is depending on the setting of DTSENSITIVE. The parameter is not a taste parameter but needs to be tuned for a specific device. Valid range: [0.0 .. 7999.0]", 0.0, 7999.0),
    "PP_DTSENSITIVE": (17, 31, 1, "rw", "int32", "Tradeoff between echo suppression and doubletalk performance. A lower value prefers high echo suppression (possibly at the cost of less doubletalk performance), a higher value prefers better doubletalk performance (possibly at the cost of good echo suppression). Good doubletalk performance is only possible for hardware without much non-linearities. When the value has 2 digits, for robustness an extra near-end speech detector is used. Valid range: [0 .. 5, 10 .. 15]", 0, 5),
    "PP_ATTNS_MODE": (17, 32, 1, "rw", "int32", "Additional reduction of AGC gain during non-speech. Valid range: 0,1 (off, on)", 0,1),
    "PP_ATTNS_NOMINAL": (17, 33, 1, "rw", "float", "Amount of additional reduction during non-speech at nominal speech level. Valid range: [0.0 .. 1.0]", 0.0,1.0),
    "PP_ATTNS_SLOPE": (17, 34, 1, "rw", "float", "Determines the extra amount of suppression during non-speech when the AGC level increases at lower speech level. The extra attenuation is given by (agcgain_nominal/agcgain_current)^attns_slope. With a value of 1.0 the amount of noise in the output remains approximately the same, independent of the agc gain. Valid range: [0.0 .. 5.0]", 0.0,5.0),
}
...
class Respeaker3800(object):
    ...
    def read(self, name):
        ...
        try:
            ...
            print(e)
            response = [SERVICER_COMMAND_RETRY]
    ...
    def set_brightness(self, value):
        ...
        self.set_led_listen(self.led_listen)
    
    def set_led_listen(self, enabled):
        self.led_listen = enabled
        if enabled:
            #LED_DOA_COLOR value:(8256, 49254)
            self.write("LED_DOA_COLOR", ((self.brightness << 8), self.brightness))
        else:
            self.write("LED_DOA_COLOR", ((self.brightness << 16), self.brightness))

    def set_led_think(self):
        self.write("LED_EFFECT", [1])

    def set_led_trace(self):
        self.write("LED_EFFECT", [4])
    ...
    def is_voice(self):
        return self.read('DOA_VALUE')[1]

    @property
    def direction(self):
        return self.read('DOA_VALUE')[0]
